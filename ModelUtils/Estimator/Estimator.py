...
class Estimator(object):
    ...
    def log_likelihood(self, x, h, embedding):
        ...
        embedding = tf.Print(embedding, [tf.reduce_min(embedding), tf.reduce_max(embedding)],
                             "The embedding ts")
        samples_scores = tf.matmul(h, embedding, transpose_b=True)
        target_score = tf.Print(self.target_score_, [tf.reduce_min(self.target_score_),
                                                     tf.reduce_max(self.target_score_)],
                                "The target is")
        ...
        Z = tf.Print(Z, [tf.reduce_min(Z), tf.reduce_max(Z)], "The value of Z is")
